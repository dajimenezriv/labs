...
idle() {
  local mode=$1 every=$2 label=$3
  "$BIN" -mode "$mode" $every -instances $N -flips 0 -settle 30s >/tmp/ff-idle.$$ &
  local pid=$!
  sleep 15
  local bk; bk=$(backends)
  wait $pid
  printf '%s\t| %s\t| %s\t| %s\n' "$mode" "$label" "$(cut -d'|' -f7 /tmp/ff-idle.$$ | tr -d ' ')" "$bk"
  rm -f /tmp/ff-idle.$$
}
echo "baseline\t| -\t| -\t| $(backends)"
