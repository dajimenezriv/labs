...
const (
	snapshotSQL = `SELECT key, enabled, rollout, version FROM lab.flags`
	oneFlagSQL  = `SELECT key, enabled, rollout, version FROM lab.flags WHERE key = $1`
	headSQL     = `SELECT coalesce(max(version), 0) FROM lab.flags`
)
...
type Cache struct {
	...
	head  int64 // highest version this instance has applied
	...
	// moves past the version of another key you never heard about.
	steps []step
...
func (c *Cache) apply(fs []Flag) {
	...
	defer c.mu.Unlock()
	for _, f := range fs {
		if cur, ok := c.flags[f.Key]; !ok || f.Version > cur.Version {
			c.flags[f.Key] = f
			c.steps = append(c.steps, step{f.Key, f.Version, now})
		}
		c.head = max(c.head, f.Version)
	}
}

// arrival returns when this instance first held key at version v or newer,
// which is the honest definition of "no longer stale for this flag": a flag
// flipped twice in a row is not stale at v1 if the instance is already at v2.
func (c *Cache) arrival(key string, v int64) (time.Time, bool) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	for _, s := range c.steps {
		if s.key == key && s.version >= v {
			return s.at, true
		}
	}
...
// stale counts the flags this instance is serving at a version older than
// the one committed. This is the number that matters and it is not derivable
// from head.
func (c *Cache) stale(truth map[string]int64) int {
	c.mu.RLock()
	defer c.mu.RUnlock()
	n := 0
	for k, v := range truth {
		if c.flags[k].Version < v {
			n++
		}
	}
	return n
...
func (c *Cache) listen(ctx context.Context, dsn string, pool *pgxpool.Pool, resync bool, watchdog time.Duration) {
	...
	for ctx.Err() == nil {
		...
		for {
			...
			// read may return a version newer than the change that triggered
